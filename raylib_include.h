...
namespace Core
{
	...
	public:
		...
		Managed<RenderTexture2D> UpscaleTexture{};
	...
	public:
			...
			while (app.window && !app.window->ShouldClose()) {
				if (loop) loop();
					...
					if (app.UpscaleEnabled && app.UpscaleTexture.IsLoaded()) {
						BeginUpscaleRender(app.UpscaleTexture, (float)app.UpscaleFactor);
						ClearBackground(app.ClearBackgroundColor);
						for (auto& [_, layer] : app.m_Layers)
							layer->OnRender();
						EndUpscaleRender(app.UpscaleTexture, app.ClearBackgroundColor, [&]() {
								for (auto& [_, layer] : app.m_Layers)
		...
		static rlRectangle GetUpscaledRenderArea()
		{
			...
			auto [offsetX, offsetY, renderW, renderH] = GetUpscaledTargetArea(app.UpscaleTexture->texture.width, app.UpscaleTexture->texture.height);
			return { offsetX, offsetY, renderW, renderH };
		}
